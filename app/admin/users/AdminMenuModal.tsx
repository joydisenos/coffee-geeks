...
export default function AdminMenuModal({ cafeteriaId, cafeteriaName, onClose }: { cafeteriaId: string, cafeteriaName: string, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-[#1c140f] border border-orange-500/40 rounded-3xl p-8 w-full max-w-5xl my-8 relative shadow-[0_20px_50px_rgba(0,0,0,0.7)] animate-fade-in-up duration-300">
         ...
         <h2 className="text-2xl font-black mb-6 text-orange-400 tracking-wide border-b border-orange-900/30 pb-4">
            Cargar Menú para: <span className="text-white">{cafeteriaName}</span>
         </h2>
